...
@section('main')
  ...
  @include('back.partials.entete', ['title' => trans('back/all.dashboard') . link_to_route('adm_curbstones.create', trans('back/all.add'), [], ['class' => 'btn btn-info pull-right']), 'icon' => 'pencil', 'fil' => trans('back/all.table')])
          ...
          <tr>
              <th>
                  ...
                  <a href="#" name="title" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'curbstones.title' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
              <th>
                  Sort
                  <a href="#" name="sort" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'curbstones.sort' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th>
              {{ trans('back/all.date') }}
              <a href="#" name="created_at" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'curbstones.created_at' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
            <th>
              {{ trans('back/all.published') }}
              <a href="#" name="active" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'curbstones.active' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
                  is_main
                  <a href="#" name="is_main" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'curbstones.is_main' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th></th>
            <th></th>
  ...
  <script>
    ...
    $(function() {
        ...
        $(document).on('change', ':checkbox[name="active"]', function() {
            ...
            $.ajax({
                url: '{{ url('adm_curbstonespostactive') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_menu"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_curbstonespostis_menu') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_main"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_curbstonespostis_main') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $('a.order').click(function(e) {
            ...
            // Send ajax      
            $.get('{{ url('adm_curbstones/order') }}', { name: name, sens: tri })
            .done(function(data) {
