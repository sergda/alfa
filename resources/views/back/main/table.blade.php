@foreach ($items as $post)
    <tr {!! session('statut') == 'admin'? 'class="warning"' : '' !!}>
        ...
        <td>{!! link_to('adm_main/' . $post->slug, trans('back/all.see'), ['class' => 'btn btn-success btn-block btn']) !!}</td>
        <td>{!! link_to_route('adm_main.edit', trans('back/all.edit'), [$post->id], ['class' => 'btn btn-warning btn-block']) !!}</td>
        <td>
            {!! Form::open(['method' => 'DELETE', 'route' => ['adm_main.destroy', $post->id]]) !!}
