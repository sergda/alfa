...
@section('main')
  ...
  @include('back.partials.entete', ['title' => trans('back/all.dashboard') . link_to_route('adm_design.create', trans('back/all.add'), [], ['class' => 'btn btn-info pull-right']), 'icon' => 'pencil', 'fil' => trans('back/all.table')])
          ...
          <tr>
              <th>
                  ...
                  <a href="#" name="title" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'design.title' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
              <th>
                  Sort
                  <a href="#" name="sort" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'design.sort' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th>
              {{ trans('back/all.date') }}
              <a href="#" name="created_at" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'design.created_at' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
            <th>
              {{ trans('back/all.published') }}
              <a href="#" name="collections.active" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'design.active' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
              <th>
                  is_menu
                  <a href="#" name="design.is_menu" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'design.is_menu' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th></th>
            <th></th>
  ...
  <script>
    ...
    $(function() {
        ...
        $(document).on('change', ':checkbox[name="active"]', function() {
            ...
            $.ajax({
                url: '{{ url('adm_designpostactive') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_menu"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_designpostis_menu') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $('a.order').click(function(e) {
            ...
            // Send ajax      
            $.get('{{ url('adm_design/order') }}', { name: name, sens: tri })
            .done(function(data) {
                $('tbody').html(data.view);
                $('.link').html(data.links.replace('adm_design.(.+)&sens', name));
                $('#tempo').remove();
            })
            .fail(function() {
                $('#tempo').remove();
