@foreach ($items as $post)
    <tr {!! session('statut') == 'admin'? 'class="warning"' : '' !!}>
        ...
        <td>{!! link_to('beds/' . $post->slug, trans('back/all.see'), ['class' => 'btn btn-success btn-block btn']) !!}</td>
        <td>{!! link_to_route('adm_beds.edit', trans('back/all.edit'), [$post->id], ['class' => 'btn btn-warning btn-block']) !!}</td>
        <td>
            {!! Form::open(['method' => 'DELETE', 'route' => ['adm_beds.destroy', $post->id]]) !!}
