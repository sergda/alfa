...
{
    public function __construct(Design $post)
    {
        $this->model = $post;
    }
    ...
    {
        $query = $this->model
            ->select('design.id', 'design.created_at', 'sort', 'title', 'active', 'is_menu', 'user_id', 'slug', 'username')
            ->join('users', 'users.id', '=', 'design.user_id')
