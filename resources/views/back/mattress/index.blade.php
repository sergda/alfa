...
@section('main')
  ...
  @include('back.partials.entete', ['title' => trans('back/all.dashboard') . link_to_route('adm_mattress.create', trans('back/all.add'), [], ['class' => 'btn btn-info pull-right']), 'icon' => 'pencil', 'fil' => trans('back/all.table')])
          ...
          <tr>
              <th>
                  ...
                  <a href="#" name="title" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'mattress.title' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
              <th>
                  Sort
                  <a href="#" name="sort" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'mattress.sort' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th>
              {{ trans('back/all.date') }}
              <a href="#" name="created_at" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'mattress.created_at' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
            <th>
              {{ trans('back/all.published') }}
              <a href="#" name="active" class="order">
                <span class="fa fa-fw fa-{{ $order->name == 'mattress.active' ? $order->sens : 'unsorted'}}"></span>
              </a>
            </th>
              <th>
                  is_main
                  <a href="#" name="is_main" class="order">
                      <span class="fa fa-fw fa-{{ $order->name == 'mattress.is_main' ? $order->sens : 'unsorted'}}"></span>
                  </a>
              </th>
            <th></th>
            <th></th>
  ...
  <script>
    ...
    $(function() {
        ...
        $(document).on('change', ':checkbox[name="active"]', function() {
            ...
            $.ajax({
                url: '{{ url('adm_mattresspostactive') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_menu"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_mattresspostis_menu') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $(document).on('change', ':checkbox[name="is_main"]', function() {
            ...
            $(this).hide().parent().append('<i class="fa fa-refresh fa-spin"></i>');
            $.ajax({
                url: '{{ url('adm_mattresspostis_main') }}' + '/' + this.value,
                type: 'PUT',
        ...
        $('a.order').click(function(e) {
            ...
            // Send ajax      
            $.get('{{ url('adm_mattress/order') }}', { name: name, sens: tri })
            .done(function(data) {
                $('tbody').html(data.view);
                $('.link').html(data.links.replace('mattress.(.+)&sens', name));
                $('#tempo').remove();
            })
            .fail(function() {
                $('#tempo').remove();
